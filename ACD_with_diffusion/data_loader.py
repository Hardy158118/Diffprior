...
def _resolve_datadir_and_suffix(args, datadir: str, suffix: str):
    """
    支持两种 data_path：
    1) 目录：/path/to/dataset_dir
    2) 文件：/path/to/edges_train_springs15r1.npy  （或 loc_train_*.npy 等）
    返回：datadir(目录), suffix(比如 springs15r1)
    """
    p = getattr(args, "data_path", "") or ""
    ...
    if os.path.isdir(p):
        datadir = p
        # 目录模式下：优先用 b_simulation_type + b_suffix 组成文件后缀
        sim = getattr(args, "b_simulation_type", "") or ""
        bs  = getattr(args, "b_suffix", "") or ""
        if sim and bs:
            suffix = f"{sim}{bs}"     # 例如 springs15r1
        else:
            suffix = suffix or getattr(args, "suffix", "")
        return datadir, suffix

    # 文件模式：从文件名提取 suffix
    datadir = os.path.dirname(p) or "."
    base = os.path.basename(p)

    prefixes = [
        "edges_train_", "loc_train_", "vel_train_",
        "edges_valid_", "loc_valid_", "vel_valid_",
        "edges_test_",  "loc_test_",  "vel_test_",
    ]
    if base.endswith(".npy"):
        for pref in prefixes:
            if base.startswith(pref):
                suffix = base[len(pref):-4]  # 去掉前缀和 .npy
                return datadir, suffix

    # 兜底：保持传入的 suffix
    return datadir, suffix or getattr(args, "suffix", "")
...
def load_data(args):
    ...
    elif any(tag in args.suffix for tag in ["LL", "LI", "CY", "BF", "TF", "BF-CV"]):
        train_loader, valid_loader, test_loader = load_genetic_data(
            args,
            suffix=args.suffix,
            batch_size=args.batch_size_multiGPU,
...
def load_springs_data_customized(args, batch_size=1, suffix="", datadir="data"):
    """Based on https://github.com/ethanfetaya/NRI (MIT License)."""
    # 统一解析：支持 data_path 是目录或文件
    datadir, suffix = _resolve_datadir_and_suffix(args, datadir, suffix)
    print(f"Loading data from {datadir} (suffix={suffix})")
