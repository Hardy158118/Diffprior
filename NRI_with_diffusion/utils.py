...
def load_customized_springs_data(args):
    ...
    # ----------------------------
    # Stress hooks (Missing/Noise)
    # loc/vel shape here is typically [S, T, D, N] (based on later transpose [0,3,1,2])
    # time dimension is axis=1
    # ----------------------------
    stress_seed = getattr(args, "stress_data_seed", args.seed)
    rng = np.random.RandomState(stress_seed)

    # (A) Noise: add AFTER normalization to [-1,1]
    noise_std = float(getattr(args, "stress_noise_std", 0.0))
    if noise_std > 0:
        loc_train = loc_train + rng.normal(0.0, noise_std, size=loc_train.shape).astype(loc_train.dtype)
        vel_train = vel_train + rng.normal(0.0, noise_std, size=vel_train.shape).astype(vel_train.dtype)

        loc_valid = loc_valid + rng.normal(0.0, noise_std, size=loc_valid.shape).astype(loc_valid.dtype)
        vel_valid = vel_valid + rng.normal(0.0, noise_std, size=vel_valid.shape).astype(vel_valid.dtype)

        loc_test  = loc_test  + rng.normal(0.0, noise_std, size=loc_test.shape).astype(loc_test.dtype)
        vel_test  = vel_test  + rng.normal(0.0, noise_std, size=vel_test.shape).astype(vel_test.dtype)

    # (B) Missing: random MISSING TIMESTEPS (recommended)
    missing_rate = float(getattr(args, "stress_missing_rate", 0.0))
    if missing_rate > 0:
        keep_prob = 1.0 - missing_rate

        def _apply_time_missing(loc, vel):
            # mask shape: [S, T, 1, 1] -> broadcast over (D, N)
            mask = rng.binomial(1, keep_prob, size=(loc.shape[0], loc.shape[1], 1, 1)).astype(loc.dtype)
            return loc * mask, vel * mask

        loc_train, vel_train = _apply_time_missing(loc_train, vel_train)
        loc_valid, vel_valid = _apply_time_missing(loc_valid, vel_valid)
        loc_test,  vel_test  = _apply_time_missing(loc_test,  vel_test)
