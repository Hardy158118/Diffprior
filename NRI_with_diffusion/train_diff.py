...
parser.add_argument('--diff-T', type=int, default=200,
...
parser.add_argument('--diff-refine-clip', type=float, default=None,
                    help='Optional clip for z0_pred during refinement (stability).')
...
def train(epoch, best_val_loss):
    ...
    for batch_idx, (data, relations) in enumerate(train_loader):
        ...
        # --- diffusion refinement: logits -> logits_refined -> decoder ---
        if args.use_diff_prior and diff_prior is not None:
            # one-step refinement: choose a small t by default if user doesn't specify
            refine_t = args.diff_refine_t_start if args.diff_refine_t_start is not None else min(30, args.diff_T)

            logits_used = diff_prior.refine(
                logits,
                t_start=refine_t,                       # now interpreted as t_ref
                deterministic=(not args.diff_refine_stochastic),   # scheme B: keep default deterministic
                clip_denoised=args.diff_refine_clip,
                gamma=args.diff_refine_gamma,
                noise_seed=args.seed,                   # fixed noise => stable decoder input across train/val/test
            )

        else:
            logits_used = logits

        edges = gumbel_softmax(logits_used, tau=args.temp, hard=args.hard)
        prob = my_softmax(logits_used, -1)
        ...
        if args.use_diff_prior and diff_prior is not None:
            ...
            # 你总loss里用的那一项（保持你现在的写法：loss = loss_nll + loss_kl）
        ...
        acc = edge_accuracy(logits_used, relations)
    ...
    with torch.no_grad():
        for batch_idx, (data, relations) in enumerate(valid_loader):
            ...
            if args.use_diff_prior and diff_prior is not None:
                # one-step refinement: choose a small t by default if user doesn't specify
                refine_t = args.diff_refine_t_start if args.diff_refine_t_start is not None else min(20, args.diff_T)
            ...
            edges = gumbel_softmax(logits_used, tau=args.temp, hard=True)
            prob = my_softmax(logits_used, -1)
            ...
            if args.use_diff_prior and diff_prior is not None:
                ...
                loss_kl = args.lambda_diff * loss_diff  # 仅用于日志/显示；保存最优仍然只看 nll_val
...
# one-step refinement: choose a small t by default if user doesn't specify
    ...
    if args.save_probs:
        ...
        np.save(os.path.join(res_folder, 'edges_test.npy'), np.concatenate(prob_test))
